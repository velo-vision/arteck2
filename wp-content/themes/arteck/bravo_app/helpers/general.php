...
if(!function_exists('bravo_custom_field_types'))
{
    function bravo_custom_field_types()
    {
        $types= array(
            array(
                'value'=>'text',
                'label'=>esc_html__('Text','bs-smarty')
            ),
            array(
                'value'=>'textarea',
                'label'=>esc_html__('Textarea','bs-smarty')
            ),
            array(
                'value'=>'date',
                'label'=>esc_html__('Date','bs-smarty')
            ),
            array(
                'value'=>'color',
                'label'=>esc_html__('Colorpicker','bs-smarty')
            ),
            array(
                'value'=>'on-off',
                'label'=>esc_html__('On/Off','bs-smarty')
            ),
        );
...
if ( ! function_exists( 'bravo_entry_meta' ) ) :
    ...
    function bravo_entry_meta() {
       echo sprintf('<a href="%s" class="readmore" role="button">'.esc_html__('Read More','bs-smarty').'</a>',get_permalink());
        ?>

            <div class="article-meta-wrapper">
                <span class="article-meta"><?php esc_html_e('By','bs-smarty')?> <a href="<?php echo get_author_posts_url( get_the_author_meta( 'ID' ) ); ?>" title="<?php the_author_meta( 'display_name' ); ?>"><?php the_author_meta( 'display_name' ); ?></a></span>
                <span class="article-meta"><?php echo get_the_category_list(', ')?></span>
                <span class="article-meta"><?php
                    $view=bravo_get_post_view();
                    if($view>1){
                        printf(esc_html__('%d Views','bs-smarty'),$view);
                    }else{
                        printf(esc_html__('%d View','bs-smarty'),$view);
                    }
    ...
    function bravo_comment_nav()
    {
        ...
        if ( get_comment_pages_count() > 1 && get_option( 'page_comments' ) ) :
            ...
            <nav class="navigation comment-navigation" role="navigation">
                <h2 class="screen-reader-text"><?php esc_html_e( 'Comment navigation', 'bs-smarty' ); ?></h2>
                <div class="nav-links">
                    <?php
                    if ( $prev_link = get_previous_comments_link(esc_html__( 'Older Comments', 'bs-smarty' ) ) ) :
                        printf( '<div class="nav-previous">%s</div>', $prev_link );
                    endif;

                    if ( $next_link = get_next_comments_link(esc_html__( 'Newer Comments', 'bs-smarty' ) ) ) :
...
if(!function_exists('bravo_breadcrumb'))
{
    function bravo_breadcrumb()
    {
        if (!is_home()) {
            if(is_404()){
                echo "<ul class='le-breadcrumb breadcrumb light'>";
            }else{
                echo "<ul class='le-breadcrumb breadcrumb'>";
            }
            echo '<li><a href="';
            echo esc_url(home_url());
            echo '">';
            echo esc_html__('Home','bs-smarty');
            echo "</a></li>  ";
            if(is_archive()){
            ...
            }elseif(is_single() or is_page())
            {
                ...
                echo "<li class='active'>".esc_html__('404 Page','bs-smarty')."</li>";
            }
            echo "</ul>";
        }else{
            ...
            echo '">';
            echo esc_html__('Home','bs-smarty');
            echo "</a></li><li class='active'><a>".esc_html__('Blog','bs-smarty')."</a></li>  ";
            echo "</ul>";
        }
    }
}
...
if(!function_exists('bravo_get_order_list'))
{
    ...
    {
        $default=array(
            'none'=>esc_html__('None','bs-smarty'),
            'ID'=>esc_html__('Post ID','bs-smarty'),
            'author'=>esc_html__('Author','bs-smarty'),
            'title'=>esc_html__('Post Title','bs-smarty'),
            'name'=>esc_html__('Post Name','bs-smarty'),
            'date'=>esc_html__('Post Date','bs-smarty'),
            'modified'=>esc_html__('Last Modified Date','bs-smarty'),
            'parent'=>esc_html__('Post Parent','bs-smarty'),
            'rand'=>esc_html__('Random','bs-smarty'),
            'comment_count'=>esc_html__('Comment Count','bs-smarty'),
        );
    ...
    function bravo_get_sub_header_template()
    {
        ...
        if($data['top_header_dropdown_position'])
        {
            BravoAssets::add_css('
                #header-top .dropdown-menu{
                    top:'.$data['top_header_dropdown_position'].'
                }
            ');
        }
        if($data['top_header_color'])
        {
            BravoAssets::add_css('
                ...
                .top_menu_dropdown a
                {
                    color:'.$data['top_header_color'].'
                }

                #header-top .search-form input{
                    border-left-color:'.$data['top_header_color'].'
                }
                #header-top .search-form:after{
                    background-color:'.$data['top_header_color'].'
                }
...
if(!function_exists('bravo_get_list_taxonomy_id'))
{
    function bravo_get_list_taxonomy_id($tax = 'category', $array = array())
    {
        ...
        $r[esc_html__('All Categories', 'bs-smarty')] = 0;
    ...
    if(!function_exists('bravo_get_list_menu')){
        function bravo_get_list_menu(){
            ...
            $r[0]['value']='';
            $r[0]['label']=esc_html__("-- Select --",'bs-smarty');
            if (!empty($allMenu)) {
...
if(!function_exists('bravo_vc_post_dropdown')){
    function  bravo_vc_post_dropdown($arg=array()){
        ...
        $return=array();
        $return[esc_html__('-- Select --','bs-smarty')]=false;
    ...
    if(!function_exists('bravo_paginate_links')){
        function bravo_paginate_links( $args = '' ,$key='paged') {
            ...
            $defaults = array(
                ...
                'show_all' => false,
                'prev_next' => true,
                'prev_text' =>esc_html__('&laquo; Previous','bs-smarty'),
                'next_text' =>esc_html__('Next &raquo;','bs-smarty'),
                'end_size' => 1,
                'mid_size' => 2,
                'type' => 'plain',
            ...
            for ( $n = 1; $n <= $total; $n++ ) :
                if ( $n == $current ) :
                    ...
                    elseif ( $dots && ! $args['show_all'] ) :
                        $page_links[] = '<span class="page-numbers dots">' .esc_html__( '&hellip;','bs-smarty' ) . '</span>';
                        $dots = false;
                    endif;
                endif;
            endfor;
