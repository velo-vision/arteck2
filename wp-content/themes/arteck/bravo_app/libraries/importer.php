        ...
        static function _do_import()
        {
            if(isset($_REQUEST['bravo_do_import']) and $_REQUEST['bravo_do_import'])
            {
                ...
                if(!current_user_can('manage_options')){
                    echo json_encode(array('status'=>0,'message'=>esc_html__('You do not have permission to do this','bs-smarty')));die;
                }
                ...
                if ( !class_exists( 'WP_Importer' ) or ! class_exists( 'WP_Import' ) ){
                    echo json_encode(array('status'=>0,'message'=>esc_html__('Importer Class Was Not Installed','bs-smarty')));die;
                }
                ...
                if($step==1) {
                    ...
                    if(!is_wp_error($data_res))
                    {
                        ...
                        if(!empty($options)){
                            ...
                            if(!function_exists('ot_options_id'))
                            {
                                echo json_encode( array(
                                        'status'   =>0,
                                        'messenger'=>__("<span class='red'>Plugin: Option Tree must be installed first. Stop working!</span>",'bs-smarty'),
                                        'next_url' => ''
                                    )
                                );
                                die;
                            ...
                            echo json_encode( array(
                                    'status'   =>"ok",
                                    'messenger'=>__("Importing the demo theme options... <span>DONE!</span><br>",'bs-smarty'),
                            ...
                            echo json_encode( array(
                                    'status'   =>0,
                                    'messenger'=>__("<span class='red'>File: theme_options.json contain NULL content. Stop working!</span>",'bs-smarty'),
                                    'next_url' => ''
                                )
                            );
                        }
                        ...
                        echo json_encode( array(
                                'status'   =>0,
                                'messenger'=>sprintf(__("<span class='red'>Can not read theme_options.json<br>File:%s<br>. Stop working!</span>",'bs-smarty'),$data_json),
                                'next_url' => ''
                            )
                        );
                        die;
                ...
                if($step==2){
                    ...
                    echo json_encode( array(
                            'status'   =>1,
                            'messenger'=>__("Importing the demo widgets... <span>DONE!</span>.<br>",'bs-smarty'),
                ...
                if($step==3){
                    ...
                    if(!$file_name){
                        echo json_encode( array(
                                'status'   =>0,
                                'messenger'=>__("<span class='red'>File Not Found. Stop working!</span>",'bs-smarty'),
                                'next_url' => ''
                            )
                        );
                        die;
                    ...
                    echo json_encode( array(
                            'status'   =>1,
                            'messenger'=>sprintf(__("Importing data: %s  of %d ... <span>DONE!</span><br>",'bs-smarty'),basename($file_name)." ".($stt_file+1),(count($ds_file))),
                            'next_url' => $nexturl,
                            'file'     => $ds_file,
                        )
                    );
                ...
                if($step==4) {
                    ...
                    echo json_encode(array(
                            'status' => 1,
                            'messenger' => __("Importing menu settings ... <span>DONE!</span><br>", 'bs-smarty'),
                            'next_url' => $nexturl,
                        )
                    );
                }
                ...
                if($step == '5'){
                    ...
                    echo json_encode( array(
                            'status'   =>"ok",
                            'messenger'=>__("Setting reading options... <span>DONE!</span><br/><span>All Done! Have Fun</span>",'bs-smarty'),
                            'next_url' => '',
                        )
                    );
